...
def wrapped_bn(num_features, spectral_bn=True, coeff=6.0):
    if spectral_bn:
...
def wrapped_conv(input_size, in_c, out_c, kernel_size, stride,
                 spectral_conv=True, coeff=6.0, n_power_iterations=1.0):
...
class WideBasic(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.bn1 = wrapped_bn(in_c, spectral_bn, coeff)
        self.conv1 = wrapped_conv(input_size, in_c, out_c, 3, stride,
                                  spectral_conv=spectral_conv, coeff=coeff,
...
class WideResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = wrapped_conv(input_size, 3, nStages[0], 3, strides[0],
                                  spectral_conv=self.spectral_conv,
                                  coeff=self.coeff,
        ...
        self.bn1 = wrapped_bn(nStages[3], spectral_bn=self.spectral_bn,
                                   coeff=self.coeff)
